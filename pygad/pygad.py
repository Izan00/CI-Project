...
class GA:
    ...
    def __init__(self, 
        ...
        # Check if the on_cal_pop_fitness exists.
        if not (on_cal_pop_fitness is None):
            # Check if the on_cal_pop_fitness is a function.
            if callable(on_cal_pop_fitness):
                # Check if the on_cal_pop_fitness function accepts 1 paramater.
                if (on_cal_pop_fitness.__code__.co_argcount == 1):
                    self.on_cal_pop_fitness = on_cal_pop_fitness
                else:
                    self.valid_parameters = False
                    raise ValueError("The function assigned to the on_cal_pop_fitness parameter must accept 1 parameter representing the instance of the genetic algorithm and the fitness values of all solutions.\nThe passed function named '{funcname}' accepts {argcount} parameter(s).".format(funcname=on_cal_pop_fitness.__code__.co_name, argcount=on_cal_pop_fitness.__code__.co_argcount))
            else:
                self.valid_parameters = False
                raise TypeError("The value assigned to the on_cal_pop_fitness parameter is expected to be of type function but ({on_cal_pop_fitness}) found.".format(on_fitness_type=type(on_cal_pop_fitness)))
        else:
            self.on_cal_pop_fitness=None
    ...
    def cal_pop_fitness(self):
        ...
        if self.on_cal_pop_fitness == None:
            ...
            pop_fitness = self.on_cal_pop_fitness(self.population)
